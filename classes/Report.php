...
class Report {
    ...
    public function getSignatoryAccount() {
        try {
            $sql = "SELECT * FROM signatories s INNER JOIN users u ON s.id = u.user_id WHERE u.user_type = 'signatory' AND s.status = 'active'";
    ...
    public function getDeficientSignatoryStudents($table_name, $clearance_id) {
        try {
            $sql = "SELECT * FROM  $table_name tb INNER JOIN students s ON tb.student_id  COLLATE utf8mb4_general_ci = s.student_id  COLLATE utf8mb4_general_ci WHERE tb.clearance_id = '$clearance_id' AND student_clearance_status = '0'";
            $result = $this->conn->query($sql);
            return $result->fetchAll(PDO::FETCH_ASSOC);
